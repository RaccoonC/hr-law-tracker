...
# 條文本體擷取用：只鎖定「第1條」或「第一條」作為起點錨點。
# 之所以不抓「第 X 條」的第一次出現，是因為部分法規頁面最上方會有
# 「本次修正條文之附表」「歷史沿革」等區塊，裡面會提到「第21條」
# 「第3條」等其他條號，若用寬鬆比對會誤把這些頁首雜訊當成條文起點。
# 每一部法規的本體一定是從「第一條／第1條」開始，用這個當錨點最穩固。
ARTICLE_START_PATTERN = re.compile(r"第\s*[一1]\s*條(?!\d)")
